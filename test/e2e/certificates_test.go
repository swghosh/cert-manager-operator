...
var _ = Describe("ACME Certificate", Ordered, func() {
	...
	Context("dns-01 challenge for LetsEncrypt Prod with AWS Route53 and Route ExternalCertificateRef", Label("Feature:RouteExternalCertificateRef"), func() {
		It("should expose a route with a valid certificate secret reference", func() {
			...
			By("creating new certificate")
			certDomain := "r." + appsDomain // "hello." + appsDomain: exceeds 64 bytes on CI
			certName := "letsencrypt-cert"
			cert := &certmanagerv1.Certificate{
				ObjectMeta: metav1.ObjectMeta{
					Name:      certName,
