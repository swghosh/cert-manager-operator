...
test-integration: generate manifests fmt vet setup-envtest ## Run test on an envtest server
	KUBEBUILDER_ASSETS="$(shell $(SETUP_ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(ENVTEST_ASSETS_DIR) -p path)" go test -v ./test/integration/...
