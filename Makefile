...
CHANNELS ?= "stable-v1,stable-v1.11"
...
MANIFEST_SOURCE = https://github.com/cert-manager/cert-manager/releases/download/v1.11.1/cert-manager.yaml
...
local-run: build
	OPERATOR_NAME=cert-manager-operator OPERAND_IMAGE_VERSION=$(BUNDLE_VERSION) OPERATOR_IMAGE_VERSION=$(BUNDLE_VERSION) ./cert-manager-operator start --config=./hack/local-run-config.yaml --kubeconfig=$${KUBECONFIG:-$$HOME/.kube/config} --namespace=cert-manager-operator --v=2
.PHONY: local-run
